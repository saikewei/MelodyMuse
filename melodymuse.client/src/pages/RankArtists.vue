<template>
    <div>
      ...
      <div class="ranking-container">
        ...
        <h1 class="ranking-title">
          <span>音乐人推荐榜</span><strong>Top100</strong>
        </h1>
  ...
  export default {
    ...
    methods: {
      fetchArtists() {
        axios.get('https://localhost:7223/api/artist/rank')
          .then(response => {
            // 根据粉丝数和播放次数计算排名，并选择前100名音乐人
            const sortedArtists = response.data
              .map(artist => ({
                ...artist,
                rankValue: artist.followersCount * 0.4 + artist.totalPlayCount * 0.6
              }))
      ...
      goToArtistPage(artistId) {
        ...
        this.$router.push({ name: 'ArtistPage', params: { artistId: artistId } });
      }
    },
    mounted() {
      this.fetchArtists();
